...
function Login() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch("https://backend:8000/api/login", {
        method: "POST",
        headers: {
      ...
      const usertyperes = await fetch(`https://backend:8000/api/get-user-type?token=${data.access_token}`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    const handleGoogleLogin = async (userData) => {
        try{
            ...
            const registerRes = await fetch("https://backend:8000/api/register-login-google", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
            ...
            const usertyperes = await fetch(`https://backend:8000/api/get-user-type?token=${registerData.access_token}`, {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
              // No hace falta Authorization aquí porque el backend no lo lee
