...
function GestioReserves() {
  ...
  const fetchReserves = async (filters = {}) => {
    try {
      ...
      const url = `https://192.168.10.10:8000/reserves?${params.toString()}`;
  ...
  const handleCancel = async (id) => {
    ...
    try {
      const res = await fetch(`https://192.168.10.10:8000/reserves/${id}`, {
        method: "DELETE",
        headers: {
  ...
  const handleNewReserve = async () => {
    ...
    try {
      const token = Cookies.get('token');
      const userRes = await fetch(`https://192.168.10.10:8000/check-user?email=${encodeURIComponent(newReserveEmail)}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      const res = await fetch("https://192.168.10.10:8000/reserves/programada", {
        method: "POST",
        headers: {
